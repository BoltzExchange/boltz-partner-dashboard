...
import { DenominationProvider, useDenomination, Denomination } from './DenominationContext';

function TestComponent() {
  const { denomination, setDenomination, toggleDenomination, formatValue, formatSats } = useDenomination();
  return (
    <div>
      ...
      <button onClick={() => setDenomination(Denomination.BTC)}>Set BTC</button>
      <button onClick={() => setDenomination(Denomination.SAT)}>Set SAT</button>
...
describe('DenominationContext', () => {
  ...
  it('formats BTC value as sats when in SAT mode', () => {
    ...
    expect(screen.getByTestId('formatted-btc')).toHaveTextContent('123,456,789 sats');
  });
  ...
  it('formats sats value as sats when in SAT mode', () => {
    ...
    expect(screen.getByTestId('formatted-sats')).toHaveTextContent('123,456,789 sats');
  });
  ...
  it('switches to BTC denomination', async () => {
    ...
    expect(screen.getByTestId('formatted-btc')).toHaveTextContent('1.23456789 BTC');
    expect(screen.getByTestId('formatted-sats')).toHaveTextContent('1.23456789 BTC');
  });
    ...
    function TestSmallValue() {
      ...
      return (
        <div>
          ...
          <button onClick={() => setDenomination(Denomination.BTC)}>BTC</button>
        </div>
      );
    }
